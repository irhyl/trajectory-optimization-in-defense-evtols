...
fig_3d.add_trace(go.Scatter3d(
    ...
    mode='lines',
    line=dict(color=COLORS['trajectory'], width=5),
    name='Planned Path',
    hoverinfo='text',
...
with c1:
    ...
    with tab_xy:
        ...
        fig_xy.update_layout(template="simple_white", xaxis_title="X (m)", yaxis_title="Y (m)", margin=dict(t=20, b=20))
        fig_xy.update_traces(line_color=COLORS['trajectory'])
        st.plotly_chart(fig_xy, use_container_width=True)
        
    with tab_xz:
        fig_xz = px.line(traj_df, x='x', y='z', title=None)
        fig_xz.update_layout(template="simple_white", xaxis_title="X (m)", yaxis_title="Altitude (m)", margin=dict(t=20, b=20))
        fig_xz.update_traces(line_color=COLORS['trajectory'])
        st.plotly_chart(fig_xz, use_container_width=True)

    with tab_yz:
        fig_yz = px.line(traj_df, x='y', y='z', title=None)
        fig_yz.update_layout(template="simple_white", xaxis_title="Y (m)", yaxis_title="Altitude (m)", margin=dict(t=20, b=20))
        fig_yz.update_traces(line_color=COLORS['trajectory'])
...
with c2:
    ...
    fig_cost.update_layout(template="simple_white", margin=dict(t=0, b=0, l=0, r=0), height=200, showlegend=True)
